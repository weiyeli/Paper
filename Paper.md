...
1. 一个随机数，用于确定每笔交易只能被处理一次的计数器
2. 账户当前的以太币余额
3. 账户的合约代码(若无则为空)
4. 账户的存储(默认为空)

两个账户之间的交易信息和信息的状态转换构成了以太坊系统的"状态"。以太币(Ether)是以太坊内部的加密燃料，主要用于支付交易费用。以太坊有两种类型账户：外部所有的账户（由私钥控制的）和合约账户（由合约代码控制）。外部所有的账户没有代码，人们可以通过创建和签名一笔交易从一个外部账户发送消息。每当合约账户收到一条消息，合约内部的代码就会被激活，允许它对内部存储进行读取和写入，和发送其它消息或者创建合约。
...
以太坊中的“交易”是指存储从外部帐户发送的消息的签名包。交易包含消息的接收者、用于确认发送者的签名、以太账户余额、要发送的数据以及两个值，称为Startgas和Gasprice。为了防止指数级爆炸和无限循环的代码，每个交易都需要限制执行代码所涉及的计算步骤，包括初始消息和执行期间引发的所有消息。Startgas是一个限制，Gasprice是支付矿工每个计算步骤的成本。如果执行交易的过程中，“Gas耗尽”，所有状态的变化都会回到原来的状态，但已支付的交易费用是不可收回的。如果中止执行交易时仍有Gas，则Gas将返回给发送方。创建合同有一个单独的交易类型和对应的消息类型；合约的地址是根据帐户随机数和交易数据的哈希计算的。
...
1. 检验输入的格式是否正确。
2. 计算交易费用:`cost = STARTGAS * GASPRICE`，从数字摘要中确定发送者的地址，并且从发送者的账户中减去交易费用和增加发送者的计数器的取值。如果账户余额不足，返回错误。
3. 设定初值`GAS = STARTGAS`，并根据交易中的字节数减去一定量的Gas。
